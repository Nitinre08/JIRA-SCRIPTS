...
def CfguestsWaiting= ComponentAccessor.getCustomFieldManager().getCustomFieldObjectsByName("Guests (Waiting for response)").first()
def currentField= getFieldById(getFieldChanged()) 
if(null!=underlyingIssue.getCustomFieldValue(CfguestsWaiting)) {
    currentField.setReadOnly(false)

    List<ApplicationUser> votersVal= underlyingIssue.getCustomFieldValue(CfguestsWaiting) as List<ApplicationUser>
